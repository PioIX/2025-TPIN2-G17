...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    socket.on("partidaCreada", (data) => {
        ...
        console.log("Evento partidaCreada →", data);

        // Asegurarse de que jugadores exista
        if (Array.isArray(data.jugadores)) {

            const idRival = data.jugadores.find(id => id !== miId) || null;

            console.log("👉 Rival detectado:", idRival);

            if (idRival) {
                localStorage.setItem("oponente_id", idRival);
            }
        }

        // Si ya se armó la partida (hay dos jugadores)
        if (data.ok && !data.esperando) {
        ...
        // Si soy host esperando
        if (data.esperando && data.userHost === miId) {
            setMensaje("Esperando oponente...");
        }
    });

    return () => socket.off("partidaCreada");
}, [socket]);
